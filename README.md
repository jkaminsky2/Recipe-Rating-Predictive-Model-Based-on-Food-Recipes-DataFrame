...
**5. Categorical Features:** We used OneHotEncoder to encode the categorical variables that were in the `review` column of the dataset. We found out that there are certain very frequent tags which could have a high correlation with the ratings column such as 'delicious' and 'flavorless'. We added these tags as columns to our dataframe with '0' representing that this tag wasn't present and '1' representing that the tag is present. By one-hot encoding these variables, the model can capture the relationships between different tags and their impact on the rating of the recipe.
